...
def _augment_payload(ctx: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    table_map = _ml100_index_rows(ctx.get("TABLE_EXTRACTIONS_100ML") or ctx.get("TABLE_EXTRACTIONS"))
    ...
    for doc in docs:
        ...
        if isinstance(table_row, dict):
            extraction["table_extraction"] = {
                ...
                "detected_columns": _ml100_safe_list(table_row.get("detected_columns")),
                "totals": table_row.get("totals") if isinstance(table_row.get("totals"), dict) else {},
                "line_items": _ml100_safe_list(table_row.get("line_items")),
                "tables": _ml100_safe_list(table_row.get("tables")),
            }
        ...
        components["table_extraction_100ml"] = {
            ...
            "detected_columns": _ml100_safe_list(table_row.get("detected_columns")) if isinstance(table_row, dict) else [],
        }
        ...
        if isinstance(table_row, dict):
            print(
                f"[table-100ml] {filename_label} | tables={int(table_row.get('tables_count') or 0)} | "
                f"rows={int(table_row.get('rows_total') or 0)} | cols={_ml100_safe_list(table_row.get('detected_columns'))}"
            )
    ...
    pipeline["ml100"] = {
        ...
        "tables_docs_count": len(_ml100_safe_list(ctx.get("TABLE_EXTRACTIONS_100ML") or ctx.get("TABLE_EXTRACTIONS"))),
    }
