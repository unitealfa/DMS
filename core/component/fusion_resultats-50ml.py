...
def _ml50_safe_list(value: Any) -> List[Any]:
...
def _ml50_norm_term(value: Any) -> str:
...
def _ml50_doc_key(doc_id: Any, filename: Any) -> str:
...
def _ml50_doc_aliases(doc_id: Any, filename: Any) -> List[str]:
    aliases = {_ml50_doc_key(doc_id, filename)}
    sid = str(doc_id or "").strip()
    if sid:
...
def _ml50_index_rows(rows: Any) -> Dict[str, Any]:
    out: Dict[str, Any] = {}
    for row in _ml50_safe_list(rows):
        if not isinstance(row, dict):
            continue
        key = _ml50_doc_key(row.get("doc_id"), row.get("filename"))
        out[key] = row
        if row.get("filename"):
...
def _ml50_group_rows(rows: Any) -> Dict[str, List[Dict[str, Any]]]:
    out: Dict[str, List[Dict[str, Any]]] = {}
    for row in _ml50_safe_list(rows):
        if not isinstance(row, dict):
            continue
        keys = {_ml50_doc_key(row.get("doc_id"), row.get("filename"))}
        if row.get("filename"):
            raw_fn = str(row.get("filename")).strip().lower()
...
def _ml50_pick_bm25(extractions: Dict[str, Any], key: str) -> Dict[str, Any]:
...
def _ml50_is_grammar_noise(token_row: Dict[str, Any]) -> bool:
    ...
    token = _ml50_norm_term(token_row.get("token"))
    lemma = _ml50_norm_term(token_row.get("lemma"))
...
def _ml50_build_grammar_block_map(ctx: Dict[str, Any]) -> Dict[str, Set[str]]:
    out: Dict[str, Set[str]] = {}
    for row in _ml50_safe_list(ctx.get("NLP_TOKENS")):
        if not isinstance(row, dict):
            continue
        if not _ml50_is_grammar_noise(row):
            continue
        token = _ml50_norm_term(row.get("token"))
        lemma = _ml50_norm_term(row.get("lemma"))
        aliases = _ml50_doc_aliases(row.get("doc_id"), row.get("filename") or row.get("doc"))
        for key in aliases:
...
def _ml50_collect_blocked_terms(
    blocked_map: Dict[str, Set[str]],
    doc_id: Any,
    filename: Any,
) -> Set[str]:
    out: Set[str] = set()
    for key in _ml50_doc_aliases(doc_id, filename):
...
def _ml50_is_blocked_topic_term(term: str, blocked_terms: Set[str]) -> bool:
    if not blocked_terms:
        return False
    norm = _ml50_norm_term(term)
    if not norm:
        return True
...
def _ml50_filter_topics(topics: Any, blocked_terms: Set[str]) -> tuple[List[Dict[str, Any]], int]:
    out: List[Dict[str, Any]] = []
    seen = set()
    removed = 0

    for item in _ml50_safe_list(topics):
        ...
        if norm in seen:
            continue
        if _ml50_is_blocked_topic_term(term, blocked_terms):
            removed += 1
            continue
        seen.add(norm)
        out.append(item)
...
def _ml50_filter_chunk_topics(chunks: Any, blocked_terms: Set[str]) -> tuple[List[Dict[str, Any]], int]:
    out: List[Dict[str, Any]] = []
    removed = 0

    for chunk in _ml50_safe_list(chunks):
        if not isinstance(chunk, dict):
            continue
        row = dict(chunk)
        src_topics = _ml50_safe_list(row.get("chunk_topics")) or _ml50_safe_list(row.get("topics"))
        clean_topics, removed_here = _ml50_filter_topics(src_topics, blocked_terms)
...
def _augment_payload(ctx: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    docs = _ml50_safe_list(payload.get("documents"))
    doc_vec_map = _ml50_index_rows(ctx.get("ML50_DOC_VECTORS"))
    topic_map = _ml50_index_rows(ctx.get("ML50_TOPICS"))
    ...
    for doc in docs:
        ...
        key = _ml50_doc_key(doc_id, filename)
        blocked_terms = _ml50_collect_blocked_terms(blocked_map, doc_id, filename)
        ...
        if not doc_topics:
            ...
            for chunk in doc_chunks:
                if not isinstance(chunk, dict):
                    continue
                for item in _ml50_safe_list(chunk.get("chunk_topics")) or _ml50_safe_list(chunk.get("topics")):
                    if not isinstance(item, dict):
                        continue
                    term = str(item.get("term") or "").strip()
                    if not term:
        ...
        doc_topics, removed_doc_topics = _ml50_filter_topics(doc_topics, blocked_terms)
        document_primary_topics = (
            _ml50_safe_list(topic_row.get("document_primary_topics"))[:2]
            or _ml50_safe_list(topic_row.get("main_topics"))[:2]
        )
        document_primary_topics = [
            str(topic).strip()
            for topic in _ml50_safe_list(document_primary_topics)
            if str(topic).strip() and not _ml50_is_blocked_topic_term(str(topic), blocked_terms)
        ][:2]
        ...
        ml50_obj = {
            "embedding_method": ctx.get("ML50_EMBEDDING_METHOD"),
            "vector_dim": ctx.get("ML50_VECTOR_DIM"),
        ...
        if not isinstance(components, dict):
            components = {}
        components["tokenisation_layout_50ml"] = {
            "embedding_method": ctx.get("ML50_EMBEDDING_METHOD"),
            "vector_dim": ctx.get("ML50_VECTOR_DIM"),
            "document_primary_topics": document_primary_topics,
        ...
        components["extraction_regles_50ml"] = {
        ...
        print(
            f"[ml50-topic] {filename_label} | document_primary_topics={document_primary_topics} | "
    ...
    if not isinstance(pipeline, dict):
        pipeline = {}
    pipeline["profile"] = "pipeline50ml"
    pipeline["ml50"] = {
        "embedding_method": ctx.get("ML50_EMBEDDING_METHOD"),
        "vector_dim": ctx.get("ML50_VECTOR_DIM"),
        "doc_vectors_count": len(_ml50_safe_list(ctx.get("ML50_DOC_VECTORS"))),
...
def _run() -> None:
    ...
    print(
        "[fusion-resultats-50ml] "
        f"docs={len(_ml50_safe_list(payload.get('documents')))} | "
        f"doc_vectors={len(_ml50_safe_list(ctx.get('ML50_DOC_VECTORS')))} | "
        f"bm25_docs={len(_ml50_safe_list(ctx.get('BM25_RESULTS')))}"
    )
