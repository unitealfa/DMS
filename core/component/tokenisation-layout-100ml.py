        ...
        try:
            with torch.inference_mode():
                for i in range(0, len(clean_texts), self.batch_size):
                    ...
                    rows = pooled.detach().cpu().numpy().astype(np.float32)
                    for row in rows:
                        out.append(row)
        except Exception as exc:
            self.error = str(exc)
...
def _augment_with_ml100(ctx: Dict[str, Any]) -> None:
    ...
    doc_key_to_chunk_vecs: Dict[str, List[np.ndarray]] = {}
    ...
    for i, row in enumerate(chunk_rows):
        vec = chunk_vecs[i] if i < len(chunk_vecs) else np.zeros(dim, dtype=np.float32)
