...
_GENERIC_SENT_END_RE = re.compile(r"(?<=[\.\!\?…؛:])(\s+|$)", flags=re.UNICODE)
...
def _split_sentence_fallback(text: str):
    if not text:
        return []
    chunks = []
    last = 0
    for m in _GENERIC_SENT_END_RE.finditer(text):
        end = m.end()
        chunks.append(text[last:end])
        last = end
    if last < len(text):
...
def split_punkt_layout(text: str, lang_pickle_name: str):
    ...
    try:
        ...
        tok = None
    if tok is None:
        return _split_sentence_fallback(text)
